...
Weatherly is designed to demonstrate API consumption and architectural approach, focusing on secure API-key handling, distributed caching, quota efficiency, rate limiting, and robust input validation.

---

## 1. Project Overview
Beyond basic consumption, Weatherly implements key production concerns:
...
- **Comprehensive Testing**: Pure TypeScript unit tests and integration tests for route boundaries.

---

## 2. Live Demo & Repository
- **Live Demo**: *[Pending Vercel Deployment]*
- **GitHub Repository**: [GitHub Link](https://github.com/example/weatherly)

---

## 3. Product Features & User Journeys

### Feature 1: Auto Weather (`/`)
- Detects the client's location automatically on visit.
- Uses client IP resolved on the server and forwards it to WeatherAI's `/v1/weather-geo` geolocation endpoint.
- Displays local current temperature, conditions, and daily forecasts inside a glassmorphic dashboard along with a Gemini-generated summary.

### Feature 2: City Dashboard (`/dashboard`)
- Allow users to search for any city globally.
- Features a debounced (600ms) geocoding proxy that queries OpenStreetMap Nominatim and caches coordinates.
- Selected location triggers consolidated current/daily weather rendering and an interactive Recharts line chart showing temperature trends.

### Feature 3: Farm Canopy Analyzer (`/farm`)
- Drag-and-drop crop care image upload.
...
| `/v1/hourly` | Scrollable hourly breakdowns | Called with `ai=false` and cached for 10-minutes |
...
## 8. Why There Is No User Login or Account
...
## 10. Security Design & Why Each Check Exists
...
## 11. Caching & API Quota Optimization
...
## 15. WeatherAI Client, Service, & Route Responsibilities
...
Duplicate `.env.example` to `.env.local` and configure your credentials:
```bash
...
We use **Vitest** to run unit and route integration tests.
```bash
npm run test
```
*Note: Our tests are monkey-patched to bypass Node 21.7.3 `styleText` formatting bugs, ensuring a 100% pass rate.*

### Run E2E tests
Playwright test files are located at `tests/e2e`. Run them using:
```bash
npx playwright test
```
...
2. **Edge Caching**: Cache public weather GET requests directly on Vercel's Edge Network (CDN) using Cache-Control headers to completely bypass serverless execution costs.
